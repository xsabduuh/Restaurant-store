...
earnedXP += (completions[dateStr] || []).length * 10;
...
const allDates = Array.from(new Set(Object.keys(completions).concat(Object.keys(financialRecords))));
...
saveTasks();
for (let d in completions) completions[d] = completions[d].filter(function(tid) { return tid !== id; });
saveCompletions();
renderTasks();
updateAllStats();
}
...
completions[today] = completions[today].filter(function(id) { return id !== taskId; });
}
saveCompletions();
updateAllStats();
renderTasks();
...
cb.onchange = function(e) { toggleTask(task.id, e.target.checked); };
...
{ id: ‘first_task’, name: ‘أول خطوة’, desc: ‘إنجاز أول مهمة’, icon: ‘fa-star’, condition: function() { return getTotalCompletedTasks() >= 1; }, progress: function() { return Math.min(1, getTotalCompletedTasks()); }, max: 1 },
{ id: ‘streak_7’, name: ‘أسبوع متواصل’, desc: ‘تسلسل 7 أيام متتالية’, icon: ‘fa-calendar-check’, condition: function() { return calculateStreak() >= 7; }, progress: function() { return Math.min(7, calculateStreak()); }, max: 7 },
{ id: ‘xp_1000’, name: ‘صائد النقاط’, desc: ‘1000 XP في شهر’, icon: ‘fa-trophy’, condition: function() { return getCurrentMonthStats().earnedXP >= 1000; }, progress: function() { return Math.min(1000, getCurrentMonthStats().earnedXP); }, max: 1000 },
{ id: ‘financial_goal’, name: ‘المحقق المالي’, desc: ‘تحقيق الهدف المالي لشهر’, icon: ‘fa-chart-line’, condition: function() { return getCurrentMonthStats().financialPercent >= 100; }, progress: function() { return getCurrentMonthStats().financialPercent; }, max: 100 },
{ id: ‘month_pass’, name: ‘بطل الشهر’, desc: ‘إنهاء شهر بنجاح’, icon: ‘fa-crown’, condition: function() {
var history = JSON.parse(localStorage.getItem(STORAGE.HISTORY) || ‘[]’);
if (history.length === 0) return false;
return history[history.length-1].passed;
}, progress: function() { return 0; }, max: 1 },
{ id: ‘task_master’, name: ‘سيد المهام’, desc: ‘إنجاز 100 مهمة إجمالاً’, icon: ‘fa-tasks’, condition: function() { return getTotalCompletedTasks() >= 100; }, progress: function() { return Math.min(100, getTotalCompletedTasks()); }, max: 100 },
{ id: ‘savings_king’, name: ‘ملك الادخار’, desc: ‘توفير 10,000 ₿ إجمالاً’, icon: ‘fa-piggy-bank’, condition: function() { return getTotalSavings() >= 10000; }, progress: function() { return Math.min(10000, getTotalSavings()); }, max: 10000 },
{ id: ‘streak_30’, name: ‘أسطورة التسلسل’, desc: ‘تسلسل 30 يوماً’, icon: ‘fa-fire’, condition: function() { return calculateStreak() >= 30; }, progress: function() { return Math.min(30, calculateStreak()); }, max: 30 },
{ id: ‘xp_5000’, name: ‘صائد النقاط الأسطوري’, desc: ‘5000 XP في شهر’, icon: ‘fa-dragon’, condition: function() { return getCurrentMonthStats().earnedXP >= 5000; }, progress: function() { return Math.min(5000, getCurrentMonthStats().earnedXP); }, max: 5000 },
{ id: ‘secret_perfect’, name: ‘الإنجاز السري: الكمال’, desc: ‘أكمل جميع المهام لمدة 7 أيام متتالية’, icon: ‘fa-gem’, condition: function() {
var count = 0;
...
return count === 7;
}, progress: function() { return 0; }, max: 1, secret: true },
{ id: ‘secret_early’, name: ‘الإنجاز السري: الفجر’, desc: ‘أضف مهمة قبل الساعة 6 صباحاً’, icon: ‘fa-sun’, condition: function() {
var lastAdd = localStorage.getItem(‘last_early_task’);
if (!lastAdd) return false;
return new Date(parseInt(lastAdd)).getHours() < 6;
}, progress: function() { return 0; }, max: 1, secret: true }
];

function checkAchievements() {
var changed = false;
achievementsList.forEach(function(ach) {
...
setTimeout(function() { alert(’🏅 إنجاز جديد: ’ + ach.name + ‘\n’ + ach.desc); }, 100);
}
});
if (changed) {
saveAchievements();
...
const xpData = last7.map(function(d) { return (completions[d] || []).length * 10; });
const financeData = last7.map(function(d) { return financialRecords[d] || 0; });
const completionData = last7.map(function(d) { return (completions[d] || []).length; });
...
if (container) {
const allDates = Array.from(new Set(Object.keys(completions).concat(Object.keys(financialRecords)))).sort().reverse();
if (allDates.length === 0) {
container.innerHTML = ‘<p>لا توجد سجلات بعد</p>’;
} else {
container.innerHTML = ‘’;
allDates.slice(0, 30).forEach(function(date) {
...
function initCharts() {
const ctx1 = document.getElementById(‘xpTrendChart’) ? document.getElementById(‘xpTrendChart’).getContext(‘2d’) : null;
const ctx2 = document.getElementById(‘financeTrendChart’) ? document.getElementById(‘financeTrendChart’).getContext(‘2d’) : null;
const ctx3 = document.getElementById(‘completionChart’) ? document.getElementById(‘completionChart’).getContext(‘2d’) : null;
...
if (completions[date].length > 0) data[date] = completions[date].length;
}
...
if (darkToggle) darkToggle.addEventListener(‘change’, function(e) { settings.darkMode = e.target.checked; applySettings(); });
if (soundToggle) soundToggle.addEventListener(‘change’, function(e) { settings.soundEnabled = e.target.checked; applySettings(); });
if (colorSelect) colorSelect.addEventListener(‘change’, function(e) { settings.colorScheme = e.target.value; applySettings(); });
if (fontSizeSelect) fontSizeSelect.addEventListener(‘change’, function(e) { settings.fontSize = e.target.value; applySettings(); });
if (animationSpeedSelect) animationSpeedSelect.addEventListener(‘change’, function(e) { settings.animationSpeed = e.target.value; applySettings(); });
if (reminderTimeInput) reminderTimeInput.addEventListener(‘change’, function(e) { settings.reminderTime = e.target.value; saveSettings(); scheduleDailyReminder(); });
...
if (confirm(‘سيتم مسح كل البيانات نهائياً!’)) { localStorage.clear(); location.reload(); }
});
var shareBtn = document.getElementById(‘shareAllAchievementsBtn’);
if (shareBtn) shareBtn.addEventListener(‘click’, shareAllAchievements);
var tourBtn = document.getElementById(‘quickTourBtn’);
if (tourBtn) tourBtn.addEventListener(‘click’, quickTour);
...
function exportData() {
const data = { tasks, completions, financialRecords, monthStartDate, financialGoal, unlockedAchievements, history: localStorage.getItem(STORAGE.HISTORY) };
...
const parts = settings.reminderTime.split(’:’);
...
recognition.onerror = function() { voiceBtn.innerHTML = ‘<i class="fas fa-microphone"></i>’; };
}
...
else { updateCharts(); updateStatsPage(); updateCalendarHeatmap(); }
if (!calHeatmapInstance) initCalendarHeatmap();
...
setGreeting();
if (currentPage === ‘stats’) {
if (!xpChart) initCharts(); else updateCharts();
updateStatsPage();
updateCalendarHeatmap();
}
if (currentPage === ‘achievements’) renderAchievements();
...
var setGoalBtn = document.getElementById('setGoalBtn');
if (setGoalBtn) setGoalBtn.addEventListener('click', function() {
    var val = parseFloat(document.getElementById('financialGoalInput').value);
    if (!isNaN(val) && val > 0) setFinancialGoal(val);
});

var recordSavingBtn = document.getElementById('recordSavingBtn');
if (recordSavingBtn) recordSavingBtn.addEventListener('click', function() {
    var val = parseFloat(document.getElementById('dailySavingInput').value);
    if (!isNaN(val) && val > 0) {
        recordSaving(val);
        document.getElementById('dailySavingInput').value = '';
    }
});

var resetTodaySavingBtn = document.getElementById('resetTodaySavingBtn');
if (resetTodaySavingBtn) resetTodaySavingBtn.addEventListener('click', function() {
